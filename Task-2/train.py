    ...
    for epoch in range(epochs):
        ...
        if validator:
            ...
            if val_ssim > best_ssim:
                best_ssim = val_ssim
                filename_pth = 'runs/last_audio_denoising_ssim_{:.4f}_psnr_{:.4f}_epoch_{}.pth'.format(
...
if __name__ == '__main__':
    ...
    train_loader = load('train', batch_size, validate = False)
    val_loader = load('val', batch_size=1, validate=True)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    if pretrained:
        model.load_state_dict(
            torch.load('runs/image_denoising_ssim_0.5289_psnr_17.8772_epoch_3.pth', map_location=device))
